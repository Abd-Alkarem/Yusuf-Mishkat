...
function getDb() {
  if (!db) {
    ...
    // Auto-seed or force-update database from repository seed file
    if (fs.existsSync(SEED_DB_PATH)) {
      let shouldCopy = false;
      if (!fs.existsSync(DB_PATH)) {
        shouldCopy = true;
      } else {
        const seedStat = fs.statSync(SEED_DB_PATH);
        const dbStat = fs.statSync(DB_PATH);
        if (seedStat.size > dbStat.size || seedStat.mtime > dbStat.mtime) {
          shouldCopy = true;
        }
      }
      ...
      if (shouldCopy) {
        ...
        try {
          ...
          console.error("Failed to copy seed database:", e);
        }
      }
    }
