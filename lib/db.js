...
function getDb() {
  if (!db) {
    ...
    // Always seed database from the repo's data.db on startup
    // This ensures Railway always uses the latest content from git
    if (fs.existsSync(SEED_DB_PATH)) {
      try {
        fs.copyFileSync(SEED_DB_PATH, DB_PATH);
        console.log("Seeding / updating database from seed data.db...");
      } catch (e) {
