...
function getDb() {
  if (!db) {
    ...
    // Auto-seed database if missing or out of date
    if (fs.existsSync(SEED_DB_PATH)) {
      try {
        if (!fs.existsSync(DB_PATH) || fs.statSync(SEED_DB_PATH).mtime > fs.statSync(DB_PATH).mtime) {
          fs.copyFileSync(SEED_DB_PATH, DB_PATH);
          console.log("Database seeded successfully from seed data.db");
        }
      } catch (e) {
        console.error("Failed to copy seed database, using fallback:", e);
      }
    }

    const targetDbPath = fs.existsSync(DB_PATH) ? DB_PATH : SEED_DB_PATH;
